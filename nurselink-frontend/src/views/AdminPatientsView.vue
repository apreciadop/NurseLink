...
watch([searchTerm, activeFilter, statusFilter], () => {
  resetCurrentPage()
})
...
<template>
  <section class="admin-patients">
      ...
      <section class="patients-tablecard">
        <header class="patients-tablehead">
          <div class="patients-search">
            <input v-model="searchTerm" type="text" class="app-input" placeholder="Search patients or phone number..." aria-label="Search patients" />
          </div>

          <button type="button" class="app-button app-button-primary patients-addbutton" @click="openCreatePatientModal">+ Add Patient</button>
        </header>

        <section class="patients-filtersrow">
          <div class="patients-filtergroup app-filter-group">
          ...
          <div class="patients-filtergroup patients-statusfilter app-filter-group">
                ...
                <tr v-for="patient in paginatedPatients" :key="patient.patientId">
                  ...
                  <td class="patients-col-center">
                    <div class="patients-actions">
                      <button type="button" class="app-action-button" :disabled="!!patient.assignedNurseId" :aria-label="`Assign patient ${patient.name} ${patient.surname}`" :title="patient.assignedNurseId ? 'Patient already assigned' : 'Assign patient'" @click="openAssignModal(patient)">
                        <img src="/icons/assignmentBlack.png" alt="" class="app-action-icon" />
                      </button>

                      <router-link :to="`/admin/patients/${patient.patientId}`" class="app-action-button" :aria-label="`Edit patient ${patient.name} ${patient.surname}`" title="Edit patient">
                        <img src="/icons/edit.png" alt="" class="app-action-icon" />
                      </router-link>
                    </div>
                  </td>
                </tr>
    ...
    <div v-if="isPatientModalOpen" class="patients-modal-overlay">
      <div class="patients-modal" @click.stop>
        ...
        <footer class="patients-modalfooter">
          <button type="button" class="app-button app-button-secondary" @click="closePatientModal">Cancel</button>
          <button type="button" class="app-button app-button-primary" :disabled="patientFormLoading" @click="submitCreatePatient">{{ patientFormLoading ? 'Creating...' : 'Create' }}</button>
        </footer>
      </div>
    </div>
    ...
    <AppFeedbackModal :visible="!!(successMessage || patientFormErrorMessage || assignErrorMessage)" :message="patientFormErrorMessage || assignErrorMessage || successMessage" :type="patientFormErrorMessage || assignErrorMessage ? 'error' : 'success'" @close="clearAdminPatientsFeedback" />
  </section>
</template>
