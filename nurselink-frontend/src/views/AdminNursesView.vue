<script src="../scripts/adminNursesView.js"></script>
...
<template>
  <section class="admin-nurses">
    ...
    <article v-else class="nurses-panel">
      <header class="nurses-paneltoolbar">
        <div class="nurses-panelsearch-wrapper">
          <input v-model="searchTerm" type="text" class="nurses-panelsearch-input" placeholder="Search nurses or phone number..." aria-label="Search nurses"/>
        </div>

        <button type="button" class="nurses-paneladd-button" @click="openCreateNurseModal">+ Add Nurse</button>
      </header>

      <section class="nurses-panelfilters">
        <label class="nurses-filteroption">
          ...
          <table class="nurses-table">
            ...
            <tbody>
              ...
              <tr v-for="nurse in paginatedNurses" :key="nurse.nurseId">
                ...
                <td>
                  <span :class="['nurses-statusbadge', nurse.active ? 'nurses-statusbadge-active' : 'nurses-statusbadge-inactive']">{{ nurse.active ? 'Active' : 'Inactive' }}</span>
                </td>

                <td>
                  <router-link :to="`/admin/nurses/${nurse.nurseId}`" class="nurses-action-button nurses-action-button-icon" :aria-label="`Edit nurse ${nurse.name} ${nurse.surname}`" title="Edit nurse">
                    <img src="/icons/edit.png" alt="Edit nurse" class="nurses-action-icon-image"/>
                  </router-link>
                </td>
              </tr>
            </tbody>
      ...
      <footer class="nurses-panelpagination">
        <button type="button" class="nurses-panelpagination-button" :disabled="currentPage === 1" @click="goToPreviousPage">&lt;</button>
        <span class="nurses-panelpagination-text">Page {{ currentPage }} of {{ totalPages }}</span>
        <button type="button" class="nurses-panelpagination-button" :disabled="currentPage === totalPages" @click="goToNextPage">&gt;</button>
      </footer>
    </article>
    ...
    <div v-if="isNurseModalOpen" class="nurses-modal-overlay">
      <div class="nurses-modal" @click.stop>
        ...
        <section class="nurses-modalbody">
          ...
          <p v-if="nurseFormErrorMessage" class="nurses-modalerror">{{ nurseFormErrorMessage }}</p>
        </section>

        <footer class="nurses-modalfooter">
          <button type="button" class="nurses-modalbutton nurses-modalbutton-secondary" @click="closeNurseModal">Cancel</button>

          <button type="button" class="nurses-modalbutton nurses-modalbutton-primary" :disabled="nurseFormLoading" @click="submitCreateNurse">{{ nurseFormLoading ? 'Creating...' : 'Create' }}</button>
        </footer>
      </div>
    </div>
  </section>
